...
public class MapReduceExample {
  ...
  public static void run(String instanceName, String zookeepers, String rootPassword, String args[]) throws Exception {
      ...
      ContinuousIngest.main(new String[] {"-i", instanceName, "-z", zookeepers, "-u", "root", "-p", rootPassword, "--table", "ci", "--num", "5000000", "--min", "0", "--max", Long.MAX_VALUE + "", "--maxColF", "1000", "--maxColQ", "1000", "--batchMemory", "1000000", "--batchLatency", "60000", "--batchThreads", "2"});
    ...
    try {
      // run verify map reduce job locally. This jobs looks for holes in the linked list create by continuous ingest
        ContinuousVerify.main(new String[] {"-D", "mapred.job.tracker=local", "-D", "fs.default.name=file:///", "-i", instanceName, "-z", zookeepers, "-u", "root", "-p" , rootPassword, "--table", "ci", "--output", outputDir, "--maxMappers", "4", "--reducers", "1"});
    } finally {
